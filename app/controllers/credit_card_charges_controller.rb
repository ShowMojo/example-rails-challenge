class CreditCardChargesController < ApplicationController
  ...
  def index
    ...
    @succeed = CreditCardCharge.where(paid: true).where(refunded: false)
    @failed = CreditCardCharge.where(paid: false).where(refunded: false)
    @disputed = CreditCardCharge.where(refunded: true).where(paid: false)
  end
  ...
  private
    ...
    def credit_card_charge_params
      params.require(:credit_card_charge).permit(:created, :paid, :amount, :currency, :refunded, :customer_id)
    end
end
