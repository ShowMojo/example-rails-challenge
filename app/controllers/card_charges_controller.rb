class CardChargesController < ApplicationController
  before_action :set_card_charge, only: %i[ show edit update destroy ]

  # GET /card_charges or /card_charges.json
  def index
    @card_charges = CardCharge.all
  end

  # GET /card_charges/1 or /card_charges/1.json
  def show
  end
  ...
  # POST /card_charges or /card_charges.json
  def create
  ...
  # PATCH/PUT /card_charges/1 or /card_charges/1.json
  def update
    respond_to do |format|
  ...
  # DELETE /card_charges/1 or /card_charges/1.json
  def destroy
    @card_charge.destroy
    respond_to do |format|
  ...
  private
    ...
    def card_charge_params
      params.require(:card_charge).permit(:paid, :amount, :refunded, :customer_id)
    end
end
