...
# custom credit card charges controller
class CreditCardChargesController < ApplicationController
  ...
  def index
    @cc_failed_charges = CreditCardCharge.includes(:customer).failed
    @cc_disputed_charges = CreditCardCharge.includes(:customer).disputed
    @cc_successful_charges = CreditCardCharge.includes(:customer).successful
  end
